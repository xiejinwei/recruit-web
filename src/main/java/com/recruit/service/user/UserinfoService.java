...
public class UserinfoService extends BaseService<Userinfo> {
	...
	public void updateUser(Userinfo info) {
		...
		if (user == null) {
			...
			// 简历名称
			if (info.getResumename() != null
					&& !info.getResumename().equals(user.getResumename()))
				user.setResumename(info.getResumename());
			// 工作年限
			if (info.getWorkyear() != 0
					&& info.getWorkyear() != user.getWorkyear())
				user.setWorkyear(info.getWorkyear());
			// 工作状态
			if (info.getWorkstatus() != 0
					&& info.getWorkstatus() != user.getWorkstatus())
	...
	public void updateResumename(String id, String resumename) {
		...
		if (u == null)
			throw new RBCException("用户信息传递错误");
		if (u.getType() != 0)
			throw new RBCException("你不是个人用户");
		Userinfo user = userinfoDao.findById(id, Userinfo.class);
		if (user == null)
			throw new RBCException("用户信息有错");
